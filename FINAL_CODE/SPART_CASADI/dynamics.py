...
from SPART_CASADI.kinematics import Accelerations, DiffKinematics, Kinematics
from SPART_CASADI.attitude_transformations import quat_DCM
from SPART_CASADI.utils import SkewSym, transpose, AttributeDict
...
def I_I(R0,RL,robot):
    ...
    Im=[zeros((3,3)) for i in range(robot.n_links_joints)]
...
def ID(wF0,wFm,t0,tL,t0dot,tLdot,P0,pm,I0,Im,Bij,Bi0,robot):
    ...
    #--- Mdot ---#
    #base-link Mdot
    Mdot0 = blockcat([[SkewSym(t0[:3]) @ I0, zeros((3,3))], [zeros(3,3), zeros(3,3)]])

    #Pre-allocate
    Mdot=[zeros(6,6) for i in range(n)]

    #Manipulator Mdot
    for i in range(n):
        Mdot[i]=blockcat([[SkewSym(tL[:3,i]) @ Im[i], zeros((3,3))], [zeros((3,3)), zeros((3,3))]])

    #--- Forces ---#

    #Base-link
    wq0=blockcat([[I0,zeros((3,3))],[zeros((3,3)),robot.base_link.mass*eye(3)]])@t0dot + Mdot0@t0 - wF0
    ...
    #Manipulator
    for i in range(n):
        wq[:,i]=blockcat([[Im[i],zeros((3,3))],[zeros((3,3)),robot.links[i].mass*eye(3)]])@tLdot[:,i]\
            + Mdot[i]@tL[:,i] - wFm[:,i]
...
def FD(tau0,taum,wF0,wFm,t0,tm,P0,pm,I0,Im,Bij,Bi0,u0,um,robot):
    ...
    #Pre-allocate
    M_hat=[zeros((6,6)) for i in range(n)]
    psi_hat=zeros((6,n))
    psi=zeros((6,n))
    ...
    for i in range(n-1, -1, -1):
        #Initialize
        M_hat[i]=blockcat([[Im[i],zeros((3,3))],[zeros((3,3)),robot.links[i].mass*eye(3)]])
        #Add children contributions
        child = np.nonzero(robot.con.child[:,i])[0].tolist()
        for j in child:
    ...
    #Base-link
    M_hat0=blockcat([[I0,zeros((3,3))], [zeros((3,3)),robot.base_link.mass*eye(3)]])
    #Add children contributions
    child = np.nonzero(robot.con.child_base)[0].tolist()
    for j in child:
    ...
    phi_tilde0=solve((transpose(P0)@psi_hat0),phi_hat0)
...
def MCB(I0,Im,Bij,Bi0,robot):
    ...
    #Backwards recursion
    for i in range(n-1, -1, -1):
        Mm_tilde[:,:,i]=blockcat([[Im[:,:,i],zeros((3,3))],[zeros((3,3)),robot.links[i].mass*eye(3)]])
        #Add children contributions
        child = np.nonzero(robot.con.child[:,i])[0].tolist()
        for j in child:
    ...
    #Base-link M tilde
    M0_tilde=blockcat([[I0,zeros((3,3))],[zeros((3,3)),robot.base_link.mass*eye(3)]])
    #Add children contributions
    child = np.nonzero(robot.con.child_base)[0].tolist()
    for j in child:
...
def CIM(q:np.ndarray, qdot:np.ndarray, robot:AttributeDict):
    ...
    #Compute Omega
    for i in range(n):
        Omega[:,:,i]=blockcat([[SkewSym(tL[:3,i]), zeros((3,3)) ]
            [zeros((3,3)), SkewSym(tL[:3,i])]] )

    #--- Mdot ---#
    #Base-link Mdot
    Mdot0=blockcat([[Omega0[:3,:3]@I0, zeros((3,3))] , zeros((3,6))])
    ...
    #Compute Mdot
    for i in range(n):
        Mdot[:,:,i]=blockcat([[Omega[:3,:3,i]@Im[:,:,i], zeros((3,3)) ], zeros((3,6))])
    ...
    for j in range(n):
        for i in range(n):
            if robot.con.branch[i,j]==1:
                ...
                Bdotij[:,:,i,j]=blockcat([[zeros((3,3)), zeros((3,3))] , 
    ...
    #Hi0_tilde
    for i in range(n-1, -1, -1):
        Bdot=blockcat([[zeros((3,3)), zeros((3,3))] , 
