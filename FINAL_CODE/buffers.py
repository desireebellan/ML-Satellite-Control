...
if __name__ == "__main__":  
    ...
    buffer = RecurrentBuffer(observation_size= 16, action_size=3, hidden_shape=(1,128), output_size=7,buffer_size=12000)
    
    epochs = 6 
    for epoch in range(epochs):
        data = deepcopy(initial_state)
        payload = torch.rand(1, 7)
        episode_start = 1
        for i in tqdm(range(2000)):
            tau = np.random.rand(n, 1) * 2 - 1
            data = spacecraftStep(tau0, tau, wf0, wfm, data, robot)
            observation = np.block([data["q0"][:4].flatten(), data["qm"].flatten(), data["u0"].flatten(), data["um"].flatten()])
            lstm_state = LSTMState(
                hidden = torch.rand(128,1),
                cell   = torch.rand(128,1))
            
            buffer.add(observation=observation, action=tau.reshape(1,3), lstm_state=lstm_state, episode_start=episode_start, target=payload)
            episode_start = 0
    for data in buffer:
        print(data)
