from utils import transpose, Omega
from attitude_transformations import quat_DCM
from kinematics import Kinematics, DiffKinematics, Velocities
from dynamics import I_I, FD

import numpy as np

def spacecraftStep(tau0, taum, wf0, wfm, data, robot, dt:float = 0.01, n_steps:int = 1):
    data["tau"] = np.block([[tau0],[taum]])
    data["wF"] = np.block([wf0, wfm])
    
    for i in range(n_steps):
        R0 = transpose(quat_DCM(transpose(data["q0"][:4,:]).squeeze()))
        r0 = data["q0"][4:, :]
...
def spacecraftFD(tau0, taum, wf0, wfm, q0, qm, u0, um, robot, dr:float=0.01):
