...
from attitude_transformations import Euler_DCM, quat_DCM
from utils import transpose, SkewSym
...
def Kinematics(R0,r0,qm,robot):
    ...
    TJ=zeros((4,4,n),dtype = R0.dtype)
    TL=zeros((4,4,n),dtype = R0.dtype)

    #--- Base-link ---#
    T0=np.block([[R0,r0], [zeros((1,3)),1]])
    ...
    for i in range(n):
        ...
        if cjoint.parent_link==-1:
            ...
            TJ[:,:,cjoint.id]=TL[:4,:4,cjoint.parent_link] @ cjoint.T
        
        #Transformation due to current joint variable
        if cjoint.type==1:
            #Revolute
            T_qm=np.block([[transpose(Euler_DCM(cjoint.axis,qm[cjoint.q_id])), zeros((3,1))], [zeros((1,3)),1]])
        elif cjoint.type==2:
            #Prismatic
            T_qm=np.block([[eye(3),cjoint.axis*qm(cjoint.q_id)],[zeros((1,3)),1]])
        else:
            #Fixed
            T_qm=np.block([[eye(3),zeros((3,1))],[zeros((1,3)),1]])
    ...
    RJ=zeros((3,3,n),dtype = R0.dtype)
    RL=zeros((3,3,n),dtype = R0.dtype)
    rJ=zeros((3,n),dtype = R0.dtype)
    rL=zeros((3,n),dtype = R0.dtype)
    #Pre-allocate rotation/sliding axis
    e=zeros((3,n),dtype = R0.dtype)
    #Pre-allocate other geometric quantities
    g=zeros((3,n),dtype = R0.dtype)
    ...
    for i in range(n):
        RJ[:3,:3,i]=TJ[:3,:3,i]
        rJ[:3,i]=TJ[:3,3,i]
        e[:3,i:i+1]=RJ[:3,:3,i]@robot.joints[i].axis
...
def DiffKinematics(R0,r0,rL,e,g,robot):
    ...
    #Pre-allocate Bij
    Bij=zeros((6,6,n,n),dtype = R0.dtype)
    ...
    for j in range(n):
        for i in range(n):
            if robot.con.branch[i,j]==1:
                ...
                Bij[:,:,i,j]=np.block([[eye(3), zeros((3,3))], [SkewSym(rL[:,j]-rL[:,i]), eye(3)]])
            else:
                #Links are not in the same branch
                Bij[:,:,i,j]=zeros((6,6))

    #Pre-allocate Bi0
    Bi0=zeros((6,6,n),dtype = R0.dtype)

    #Compute Bi0
    for i in range(n):
        Bi0[:,:,i]=np.block([[eye(3), zeros((3,3))], [SkewSym(r0-rL[:,i:i+1]), eye(3)]])

    #--- Twist-propagation "vector" ---#

    #Pre-allocate pm
    pm=zeros((6,n),dtype = R0.dtype)

    #Base-link
    P0=np.block([[R0,zeros((3,3))], [zeros((3,3)), eye(3)]])

    #Forward recursion to obtain the twist-propagation "vector"
    for i in range(n):
        if robot.joints[i].type==1:
            #Revolute joint
            pm[:,i:i+1]=np.block([[e[:,i:i+1]], [transpose(np.cross(e[:,i],g[:,i]))]])
        elif robot.joints[i].type==2:
            #Prismatic joint
            pm[:,i]=np.block([[zeros((3,1))], [e[:,i]]])
...
def Accelerations(t0,tL,P0,pm,Bi0,Bij,u0,um,u0dot,umdot,robot):
    ...
    #Pre-allocate
    Omegam=zeros((6,6,n),dtype=t0.dtype)

    #Compute Omega for manipulator
    for i in range(n):
        Omegam[:,:,i]=np.block([[SkewSym(tL[:3,i]), zeros((3,3))],
    ...
    #Pre-allocate
    tLdot=zeros((6,n),dtype=t0.dtype)
    ...
    for i in range(n):
        ...
        if robot.joints[i].parent_link==-1:
            #First Link
            tLdot[:,i:i+1] = Bi0[:,:,i] @ t0dot + np.block([[zeros((3,6))],[SkewSym(t0[3:]-tL[3:,i:i+1]),zeros((3,3))]]) @ t0
        else:
            #Rest of the links.
            tLdot[:,i] = Bij[:,:,i,robot.joints[i].parent_link] @ tLdot[:,robot.joints[i].parent_link] \
                + np.block([[zeros((3,6))], [SkewSym(tL[3:,robot.joints[i].parent_link]-tL[3:,i]), zeros((3,3))]]) @ tL[:,robot.joints[i].parent_link]
