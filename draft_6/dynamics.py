from kinematics import Accelerations, DiffKinematics, Kinematics, Velocities
...
def GIM(q:np.ndarray, robot:AttributeDict):
    ...
    I0, Im = I_I(R0, RL, robot)
    Bij, Bi0, P0, pm = DiffKinematics(R0=R0, r0=r0, rL=rL, e=e, g=g, robot=robot)
    M0_tilde, Mm_tilde = MCB(I0,Im,Bij,Bi0,robot)
    ...
    for i in range(n):
        if robot.joints[i].type!=0:
            iid = robot.joints[i].q_id
            H0m[:,iid:iid+1]=transpose(transpose(pm[:,i:i+1])@Mm_tilde[:,:,i]@Bi0[:,:,i]@P0)
...
def CIM(q:np.ndarray, qdot:np.ndarray, robot:AttributeDict):
    ...
    for i in range(n-1, -1, -1):
        Bdot=np.block([[zeros((3,3)), zeros((3,3))] , 
                       [SkewSym(t0[3:,:]-tL[3:,i:i+1]), zeros((3,3))]] )
        ...
        for i in range(n):
            ...
            if (robot.joints[i].type!=0 and robot.joints[j].type!=0) and (robot.con.branch[i,j]==1 or robot.con.branch[j,i]==1):
                ...
                if i<=j : 
                    ...
                    Cm[robot.joints[i].q_id,robot.joints[j].q_id] = transpose(pm[:,i:i+1])\
    ...
    for j in range(n):
        if  robot.joints[j].type!=0:
            if j==n-1:
                ...
                C0m[:,robot.joints[j].q_id]=transpose(P0)@(transpose(Bi0[:,:,j])@Mm_tilde[:,:,j]@Omega[:,:,j]\
                    +child_con + Mdot_tilde[:,:,j])@pm[:,j]
    #Cm0
    for i in range(n):
        if robot.joints[i].type!=0:
            Cm0[robot.joints[i].q_id,:]=transpose(pm[:,i:i+1])@(Mm_tilde[:,:,i]@Bi0[:,:,i]@Omega0 \
